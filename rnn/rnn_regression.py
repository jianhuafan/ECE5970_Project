...
def build_graph(model, lr, lr_decay, lr_decay_after, grad_clipping, num_hidden, num_layers, batch_size, train_seq_length,
    ...
    # loss = tf.metrics.mean_absolute_error(last_rnn_output, Y)
    # prediction, loss = tflearn.models.linear_regression(last_rnn_output, Y)
    # Softmax layer
    ...
    y_ = tf.matmul(last_rnn_output, W) + b
    # loss = tf.metrics.mean_absolute_error(y_, Y)
    loss = tf.reduce_mean(tf.keras.losses.mean_absolute_error(Y, y_))
...
def train_graph(index, load_model, model, num_layers, num_hidden, g, batch_size, train_seq_length, max_epochs, weights_fpath,
    ...
    with tf.Session() as sess:
        ...
        if load_model:
            saver.restore(sess, weights_fpath+'model_{}_{}_{}_{}.ckpt'.format(model, num_hidden, max_epochs, index))
        else:
        ...
        # mean_accuracy_list = []
        current_epoch = 1
        txt_path = "results/{}_{}_{}_{}.txt".format(model, num_hidden, max_epochs, index)
        best_loss = 10
        best_epoch = 1
        try:
            ...
            while current_epoch < (max_epochs + 1):
                ...
                seq_iter = utils.reg_sequences(index, batch_size, train_seq_length, input_train_fpath, output_train_fpath)
                batch_index = -1
                total_loss = []
                # total_accuracy = []
                iteration = 0
                for (input_data, y) in seq_iter:
                    ...
                    if input_data is not None and y is not None:
                        ...
                        loss_, s, _ = sess.run([g['loss'], g['summ'], g['ts']], feed_dict=feed)
                        with open(txt_path, 'a') as f:
                ...
                plot(mean_loss_list)
                plt.savefig("results/{}_{}_{}_{}.png".format(model, num_hidden, max_epochs, index)) 
                current_epoch += 1
                save_path = saver.save(sess, weights_fpath+'model_{}_{}_{}_{}.ckpt'.format(model, num_hidden, max_epochs, index))
...
def test_graph(index, load_model, model, num_layers, num_hidden, g, batch_size, train_seq_length, max_epochs, weights_fpath,
    input_train_fpath, input_test_fpath, output_train_fpath, output_test_fpath):
    saver = tf.train.Saver()
    with tf.Session() as sess:
        saver.restore(sess, weights_fpath+'model_{}_{}_{}_{}.ckpt'.format(model, num_hidden, max_epochs, index))
        mean_loss_list = []
        try:
            seq_iter = utils.reg_sequences(index, batch_size, train_seq_length, input_test_fpath, output_test_fpath)
            total_loss = []
            for (input_data, y) in seq_iter:
                if input_data is not None and y is not None:
                    feed = {g['X']: input_data, g['Y']: y}
